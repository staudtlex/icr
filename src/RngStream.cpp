 ...
 *
 * Note:           Minor modifications by Alexander Staudt <staudtlex@live.de>
 * Date modified:  18 January 2018
 *
...
const double invtwo24 =   5.9604644775390625e-8;
...
//-------------------------------------------------------------------------
// Compute the matrix B = (A^(2^e) Mod m).
//
...
//-------------------------------------------------------------------------
// Compute the matrix B = (A^c Mod m).
//
void MatPowModM (const double A[3][3], double B[3][3], double m, long c)
{
    int i, j;
    long n = c;
    double W[3][3];
...
RngStream::RngStream (string s) : name (s)
{
   anti = false;
   ...
   generates antithetic variates if anti = true.
...
//-------------------------------------------------------------------------
// Reset Stream to : 1. Beginning of Stream. 2. Beginning of SubStream.
// 3. NextSubStream.
//
void RngStream::Reset (ResetType where)
{
    switch (where)
    {
    case StartStream:
...
{
    Rcpp::Rcout << "The current state of the Rngstream";
    if (name.size() > 0)
        Rcpp::Rcout << " " << name;
    Rcpp::Rcout << ":\n   Cg = { ";
...
double RngStream::RandU01 ()
{
    ...
    /* Combination */
    u = ((p1 > p2) ? (p1 - p2) * norme : (p1 - p2 + m1) * norme);
...
//-------------------------------------------------------------------------
// Generate the next random number with 53 bits precision.
//
double RngStream::RandU01d ()
{
    double u;

    if (anti) {
        u = RandU01();
        u += (RandU01()  - 1.0) * invtwo24;
